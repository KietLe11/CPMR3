...
#
from kinova_gen3_interfaces.srv import Status, SetGripper, GetGripper, SetJoints, GetJoints, GetTool, SetTool, ClearFaults
import rclpy
...
class Kinova_Gen3_Interface(Node):
    ...
    def __init__(self):
        ...
        self.create_service(ClearFaults, "clear_faults", self._handle_clear_faults)
    ...
    def _handle_clear_faults(self, request, response):
        """Clear faults on the arm"""
        self.get_logger().info(f'{self.get_name()} Clearing faults...')
        try:
            self._base.ClearFaults()
            time.sleep(0.5)  # small pause to let robot recover
            response.status = True
        except Exception as e:
            self.get_logger().error(f"Failed to clear faults: {e}")
            response.status = False
        return response
