...
import torch  # ✅ FIX: to safely check for torch.Tensors
...
class YOLO_Pose(Node):
    ...
    def _to_float(self, val):  # ✅ FIX: helper to ensure native Python float
        if isinstance(val, torch.Tensor):
            return float(val.item())
        elif isinstance(val, np.generic):  # e.g., np.float32
            return float(val)
        else:
            return float(val)
        ...
        for points in results.keypoints:        
            ...
            for kp_id, (p, conf) in enumerate(zip(points.xy[0], points.conf[0])):
                if conf >= self._threshold:
                    # ✅ FIX: Convert tensor values to floats before appending
    ...
    def _camera_callback(self, data):
        ...
        if results.keypoints:
            ...
            # ✅ FIX: all values already float-safe from parse_keypoints()
            keypoint_dict = {
                YOLO_Pose._BODY_PARTS[kp[0]]: [kp[1], kp[2], kp[3]]
                for kp in keypoints
            }

            msg = String()
            msg.data = json.dumps(keypoint_dict)  # ✅ now fully JSON-safe
            self._pub.publish(msg)

            # Visualize results on frame        
            annotated_frame = results.plot()  # ✅ use results.plot(), not results[0].plot()
